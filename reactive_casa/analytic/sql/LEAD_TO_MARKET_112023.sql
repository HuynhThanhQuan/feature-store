...
T2 AS (
    ...
    AND CUST_STATUS IN ('DONG BANG', 'NGU DONG')),
T3 AS (
    SELECT T2.*, T1.SCORE
    FROM T1 INNER JOIN T2 ON T1.CUSTOMER_CDE = T2.CUSTOMER_CDE
    WHERE ROWNUM <= 10000),
T4 AS (
...
D1 AS (
    ...
    T3.CUST_TYPE, T3.CUST_STATUS, T5.CUS_OPEN_DT,
...
D4 AS (
    ...
    D1.CUST_TYPE, D1.CUST_STATUS, D1.CUS_OPEN_DT,
