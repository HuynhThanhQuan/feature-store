CREATE TABLE CINS_TMP_CUSTOMER_01042023 (
...
CREATE TABLE CINS_TMP_CUSTOMER_STATUS_01042023 (
    CUSTOMER_CDE VARCHAR2(25 BYTE),
...
CREATE TABLE CINS_TMP_CARD_DIM_01042023 (
...
/*
Table Name: CINS_TMP_CUSTOMER_01042023
Derived From: 
  DW_ANALYTICS.DW_CUSTOMER_DIM: 
    ...
    - SUB_SECTOR_CDE
*/
INSERT INTO CINS_TMP_CUSTOMER_01042023
SELECT CUSTOMER_CDE 
FROM (
...
/*
Table Name: CINS_TMP_CUSTOMER_STATUS_01042023
Derived From: 
  ...
  CINS_TMP_CUSTOMER_01042023: 
    - CUSTOMER_CDE
*/
INSERT INTO CINS_TMP_CUSTOMER_STATUS_01042023
...
FROM (
    ...
    RIGHT JOIN CINS_TMP_CUSTOMER_01042023 C ON T.CUSTOMER_CDE=C.CUSTOMER_CDE
    WHERE 
        T.PROCESS_DT = ADD_MONTHS(TO_DATE('01-04-2023', 'DD-MM-YY'), -1)
        OR T.PROCESS_DT = TO_DATE('01-04-2023', 'DD-MM-YY')
    GROUP BY 
        T.CUSTOMER_CDE, 
        T.PROCESS_DT
) A;
...
/*
Table Name: CINS_TMP_CARD_DIM_01042023
Derived From: 
  DW_ANALYTICS.DW_CARD_MASTER_DIM: 
    ...
    - PLASTIC_CDE 
*/
INSERT INTO CINS_TMP_CARD_DIM_01042023 
SELECT DISTINCT CARD_CDE 
FROM DW_ANALYTICS.DW_CARD_MASTER_DIM DIM
RIGHT JOIN CINS_TMP_CUSTOMER_01042023 C ON DIM.CUSTOMER_CDE=C.CUSTOMER_CDE
...
Derived From: 
  CINS_TMP_CUSTOMER_STATUS_01042023: 
    - CUSTOMER_CDE 
    - CUST_STT 
    - CUST_STT_CHG 
    - RPT_DT
...
FROM CINS_TMP_CUSTOMER_STATUS_01042023
WHERE CUST_STT = 2
  AND CUST_STT_CHG = 1
  AND RPT_DT = TO_DATE('01-04-2023', 'DD-MM-YY');
...
RIGHT JOIN CINS_TMP_CUSTOMER_01042023 TMP ON AMD.CUSTOMER_CDE = TMP.CUSTOMER_CDE
WHERE AMD.ACTIVE = 1
    AND AMD.COMPANY_KEY = 1
    AND AMD.SUB_SECTOR_CDE IN ('1700', '1602')
    AND AMD.CATEGORY_CDE LIKE '10__'
    AND TO_CHAR(AMD.CLOSE_DT) = '01-JAN-00'
    AND AMD.OPEN_DT <= TO_DATE('01-04-2023', 'DD-MM-YY');
...
Derived From: DW_CARD_MASTER_DIM, CINS_TMP_CUSTOMER_01042023
*/
INSERT INTO CINS_FEATURE_STORE_REACTIVATED
SELECT CUSTOMER_CDE,
...
FROM
  (SELECT DISTINCT DIM.CUSTOMER_CDE
   ...
   WHERE SUBSTR(DIM.CARD_CDE, 1, 1) = '3'
     ...
     AND DIM.ACTIVATION_DT <= TO_DATE('01-04-2023', 'DD-MM-YY'));
...
Derived From: DW_EWALL_USER_DIM, CINS_TMP_CUSTOMER_01042023
*/
INSERT INTO CINS_FEATURE_STORE_REACTIVATED
SELECT DISTINCT DIM.CUSTOMER_CDE,
...
RIGHT JOIN CINS_TMP_CUSTOMER_01042023 TMP ON DIM.CUSTOMER_CDE = TMP.CUSTOMER_CDE
WHERE DIM.FIRST_SIGNED_ON < TO_DATE('01-04-2023', 'DD-MM-YY');
...
Derived From: DW_EB_USER, CINS_TMP_CUSTOMER_01042023
*/
INSERT INTO CINS_FEATURE_STORE_REACTIVATED
SELECT DISTINCT DIM.CUSTOMER_CDE,
...
WHERE DIM.PROCESS_DT IN
    (SELECT MAX(PROCESS_DT)
     ...
     WHERE PROCESS_DT < TO_DATE('01-04-2023', 'DD-MM-YY') )
  AND DIM.DEL_FLG = 'N';
...
SELECT DIM.CUSTOMER_CDE,
  'LIFE_STG' AS FTR_NM,
  CASE
      WHEN FLOOR(MONTHS_BETWEEN(TO_DATE('01-04-2023', 'DD-MM-YY'), BIRTHDAY)/12) BETWEEN 18 AND 26 THEN 'Bắt đầu sự nghiệp'
      WHEN FLOOR(MONTHS_BETWEEN(TO_DATE('01-04-2023', 'DD-MM-YY'), BIRTHDAY)/12) BETWEEN 27 AND 35 THEN 'Lập gia đình'
      WHEN FLOOR(MONTHS_BETWEEN(TO_DATE('01-04-2023', 'DD-MM-YY'), BIRTHDAY)/12) BETWEEN 36 AND 45 THEN 'Thiết lập tài sản'
      WHEN FLOOR(MONTHS_BETWEEN(TO_DATE('01-04-2023', 'DD-MM-YY'), BIRTHDAY)/12) BETWEEN 46 AND 54 THEN 'Bảo vệ tài sản'
      WHEN FLOOR(MONTHS_BETWEEN(TO_DATE('01-04-2023', 'DD-MM-YY'), BIRTHDAY)/12) BETWEEN 55 AND 64 THEN 'Cuối sự nghiệp'
      WHEN FLOOR(MONTHS_BETWEEN(TO_DATE('01-04-2023', 'DD-MM-YY'), BIRTHDAY)/12) >= 65 THEN 'Nghỉ hưu'
      ELSE NULL
  END AS FTR_VAL,
  TO_DATE('01-04-2023', 'DD-MM-YY') AS RPT_DT,
  CURRENT_TIMESTAMP AS ADD_TSTP
...
WHERE DIM.ACTIVE = 1
  ...
  AND DIM.CUS_OPEN_DT < TO_DATE('01-04-2023', 'DD-MM-YY');
...
T1 AS (
  ...
  RIGHT JOIN CINS_TMP_CUSTOMER_01042023 T3 ON T1.CUSTOMER_CDE = T3.CUSTOMER_CDE
)
SELECT 
  CUSTOMER_CDE,
  'AREA' AS FTR_NM, 
...
T5 AS (
    SELECT 
        ...
        END CREDIT_SCORE
    FROM T3 
    RIGHT JOIN CINS_TMP_CUSTOMER_01042023 T4 ON T3.CUSTOMER_CDE = T4.CUSTOMER_CDE
...
RIGHT JOIN CINS_TMP_CUSTOMER_01042023 TMP ON CS.CUSTOMER_CDE = TMP.CUSTOMER_CDE
...
WITH 
T0 AS (
    SELECT * FROM CINS_TMP_CUSTOMER_01042023
),
T1 AS (
    SELECT * FROM DW_ANALYTICS.DWA_STMT_EBANK
    WHERE PROCESS_DT < TO_DATE('01-04-2023', 'DD-MM-YY')
    AND PROCESS_DT >= ADD_MONTHS(TO_DATE('01-04-2023', 'DD-MM-YY'), -36)
...
SELECT 
    ...
    TO_DATE('01-04-2023', 'DD-MM-YY') - NVL(MAX(PROCESS_DT), ADD_MONTHS(TO_DATE('01-04-2023', 'DD-MM-YY'), -36)) AS FTR_VAL,
    TO_DATE('01-04-2023', 'DD-MM-YY') AS RPT_DT,
    CURRENT_TIMESTAMP AS ADD_TSTP
FROM T3
...
Derived From: DATA_RPT_CARD_493, CINS_TMP_CUSTOMER_01042023, CINS_TMP_CARD_DIM_01042023
*/
INSERT INTO CINS_FEATURE_STORE_REACTIVATED
SELECT DIM.CUSTOMER_CDE,
...
WHERE SUBSTR(DIM.CARD_CDE, 1, 1) = '3'
  AND DIM.CARD_CDE IN
    (SELECT CARD_CDE FROM CINS_TMP_CARD_DIM_01042023)
  AND DIM.PROCESS_DT >= ADD_MONTHS(TO_DATE('01-04-2023', 'DD-MM-YY'), -36)
  AND DIM.PROCESS_DT < TO_DATE('01-04-2023', 'DD-MM-YY')
...
Derived From: DATA_RPT_CARD_493, CINS_TMP_CUSTOMER_01042023
*/
INSERT INTO CINS_FEATURE_STORE_REACTIVATED
SELECT CUSTOMER_CDE,
  ...
  (SELECT BAL.*, ROW_NUMBER() OVER(PARTITION BY CUSTOMER_CDE, CARD_CDE ORDER BY PROCESS_DT DESC) RN
   FROM
     (SELECT CC.CUSTOMER_CDE,
      ...
      RIGHT JOIN CINS_TMP_CUSTOMER_01042023 TMP ON CC.CUSTOMER_CDE = TMP.CUSTOMER_CDE
      WHERE CC.PROCESS_DT < TO_DATE('01-04-2023', 'DD-MM-YY')
...
SELECT CUSTOMER_CDE,
  ...
  AVG(TO_DATE('01-04-2023', 'DD-MM-YY') - TO_DATE(LAST_SIGNED_ON)) AS FTR_VAL,
  TO_DATE('01-04-2023', 'DD-MM-YY') AS RPT_DT,
  CURRENT_TIMESTAMP AS ADD_TSTP
FROM A
WHERE TO_DATE(LAST_SIGNED_ON) < TO_DATE('01-04-2023', 'DD-MM-YY')
  AND TO_DATE(LAST_SIGNED_ON) >= ADD_MONTHS(TO_DATE('01-04-2023', 'DD-MM-YY'), -36)
  AND RN = 1
GROUP BY CUSTOMER_CDE;
...
Derived From: DW_EWALL_TRANSACTION_FCT, CINS_TMP_CUSTOMER_01042023
*/
INSERT INTO CINS_FEATURE_STORE_REACTIVATED
...
RIGHT JOIN CINS_TMP_CUSTOMER_01042023 TMP ON FCT.CUSTOMER_CDE = TMP.CUSTOMER_CDE
WHERE PROCESS_DT < TO_DATE('01-04-2023', 'DD-MM-YY')
  AND PROCESS_DT >= ADD_MONTHS(TO_DATE('01-04-2023', 'DD-MM-YY'), -36)
  AND TXN_STATUS = 'S'
...
SELECT CUSTOMER_CDE,
  ...
  TO_DATE('01-04-2023', 'DD-MM-YY') - ACTIVATE_DATE AS FTR_VAL,
  TO_DATE('01-04-2023', 'DD-MM-YY') AS RPT_DT,
  CURRENT_TIMESTAMP AS ADD_TSTP
FROM
  (SELECT T1.CUSTOMER_CDE,
   ...
   RIGHT JOIN CINS_TMP_CUSTOMER_01042023 TMP ON T1.CUSTOMER_CDE = TMP.CUSTOMER_CDE
   WHERE TO_DATE(ACTIVATE_DT) <= TO_DATE('01-04-2023', 'DD-MM-YY')
