...
SELECT CUSTOMER_CDE,
       'EB_MBIB_DAY_SINCE_LTST_TXN' FTR_NM,
                                    TO_DATE('11-06-2023', 'DD-MM-YY') - NVL(MAX(TXN_DT), ADD_MONTHS(TO_DATE('11-06-2023', 'DD-MM-YY'), -36)) FTR_VAL,
                                    TO_CHAR(TO_DATE('11-06-2023', 'DD-MM-YY'), 'DD-MM-YYYY') AS RPT_DT,
                                    CURRENT_TIMESTAMP ADD_TSTP
FROM DW_ANALYTICS.DW_EB_TRANSACTION_FCT
WHERE TXN_DT >= ADD_MONTHS(TO_DATE('11-06-2023', 'DD-MM-YY'), -36)
...
Derived From: DW_EWALL_TRANSACTION_FCT, CINS_TMP_CUSTOMER_11062023
*/
INSERT INTO CINS_FEATURE_STORE_TEST
SELECT CUSTOMER_CDE,
...
Derived From: DW_EWALL_TRANSACTION_FCT, CINS_TMP_CUSTOMER_11062023, DW_EWALL_USER_DIM
*/
INSERT INTO CINS_FEATURE_STORE_TEST
SELECT CUSTOMER_CDE,
...
FROM
  (SELECT CUSTOMER_CDE,
          NVL(MONTHS_BETWEEN(LEAD(TXN_DT) OVER (PARTITION BY CUSTOMER_CDE
                                                ORDER BY TXN_DT), TXN_DT), MONTHS_BETWEEN(TO_DATE('11-06-2023', 'DD-MM-YY'), TXN_DT)) TXN_GAP
   FROM DW_ANALYTICS.DW_EWALL_TRANSACTION_FCT
...
WHERE PROCESS_DT >= ADD_MONTHS(TO_DATE('11-06-2023', 'DD-MM-YY'), -36)
  AND CARD_CDE LIKE '3%'
  AND TRAN_STATUS = 'S'
...
FROM
  (SELECT CUSTOMER_CDE,
          NVL(MONTHS_BETWEEN(LEAD(PROCESS_DT) OVER (PARTITION BY CUSTOMER_CDE
                                                    ORDER BY PROCESS_DT), PROCESS_DT), MONTHS_BETWEEN(TO_DATE('11-06-2023', 'DD-MM-YY'), PROCESS_DT)) TXN_GAP
...
Derived From: DW_ACCOUNT_MASTER_DIM, DWA_STMT_EBANK, TRANSACTION_CODE, CINS_TMP_CUSTOMER_11062023
*/
INSERT INTO CINS_FEATURE_STORE_TEST
SELECT CUSTOMER_CDE,
...
Derived From: DWA_STMT_EBANK, TRANSACTION_CODE, CINS_TMP_CUSTOMER_11062023
*/ 
INSERT INTO CINS_FEATURE_STORE_TEST
  ...
  AND CUSTOMER_CDE IN
    (SELECT CAST(AC_CUST_ID AS VARCHAR2(9)) CUSTOMER_CDE
     ...
     WHERE AC_CATEGORY LIKE '10__'
       AND NVL(AC_OPENING_DATE, AC_VALUE_DATE) < TO_DATE('11-06-2023', 'DD-MM-YY')
       AND NVL(AC_OPENING_DATE, AC_VALUE_DATE) >= ADD_MONTHS(TO_DATE('11-06-2023', 'DD-MM-YY'), -36)
     UNION SELECT CUSTOMER_CDE
     FROM CINS_FEATURE_STORE_TEST
     WHERE CAST(FTR_VAL AS NUMBER(38, 2)) > 0
       AND FTR_NM = 'CASA_SUM_BAL_NOW'
       AND RPT_DT = TO_CHAR(TO_DATE('11-06-2023', 'DD-MM-YY'), 'DD-MM-YYYY') )
  AND CUSTOMER_CDE NOT IN
    (SELECT CUSTOMER_ID
     FROM DW_ANALYTICS.DWA_STMT_EBANK TXN
     JOIN
...
*/
INSERT INTO CINS_FEATURE_STORE_TEST WITH CUST AS
  (SELECT CUSTOMER_CDE,
          COMPANY_BOOK
   FROM DW_ANALYTICS.DW_CUSTOMER_DIM
   WHERE ACTIVE = 1
...
Derived From: CINS_TMP_CUSTOMER_STATUS_11062023
*/
INSERT INTO CINS_FEATURE_STORE_TEST
SELECT CUSTOMER_CDE,
       'INACTIVE' FTR_NM,
...
Derived From: DW_EB_USER, CINS_TMP_CUSTOMER_11062023
*/
INSERT INTO CINS_FEATURE_STORE_TEST
...
Derived From: DW_EWALL_USER_DIM, CINS_TMP_CUSTOMER_11062023
*/
INSERT INTO CINS_FEATURE_STORE_TEST
SELECT DISTINCT CUSTOMER_CDE,
...
WHERE FIRST_SIGNED_ON < TO_DATE('11-06-2023', 'DD-MM-YY')
  AND CUSTOMER_CDE IN
    (SELECT CUSTOMER_CDE
     FROM CINS_TMP_CUSTOMER_11062023);
...
SELECT CUSTOMER_CDE,
       'EB_MBIB_DAY_SINCE_ACTIVE' FTR_NM,
       TO_DATE('11-06-2023', 'DD-MM-YY') - ACTIVATE_DATE FTR_VAL,
       TO_CHAR(TO_DATE('11-06-2023', 'DD-MM-YY'), 'DD-MM-YYYY') AS RPT_DT,
       CURRENT_TIMESTAMP ADD_TSTP
FROM
  (SELECT CUSTOMER_CDE,
   ...
   WHERE EXISTS
       (SELECT 1
        FROM CINS_TMP_CUSTOMER_11062023 t2
        WHERE t1.CUSTOMER_CDE=t2.CUSTOMER_CDE)
     AND TO_DATE(ACTIVATE_DT) <= TO_DATE('11-06-2023', 'DD-MM-YY')
     AND ACTIVATE_DT != TO_DATE('01/01/2400', 'DD/MM/YYYY')
   GROUP BY customer_cde) A;