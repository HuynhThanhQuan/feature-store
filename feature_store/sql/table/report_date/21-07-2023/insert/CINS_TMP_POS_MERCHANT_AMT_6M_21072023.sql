INSERT INTO CINS_TMP_POS_MERCHANT_AMT_6M_21072023 
SELECT 
    E.customer_cde, 
    F.MERCHANT_ID,  
    NULL TERMINAL_ID,
    TO_CHAR(TO_DATE('21-07-2023','DD-MM-YY'), 'DD-MM-YYYY') AS RPT_DT, 
    AMT_BILL,
    ...
    FROM (
        ...
        FROM (
            ...
            FROM (
                ...
                WHERE 
                    process_dt < TO_DATE('21-07-2023','DD-MM-YY') 
                    AND process_dt >= ADD_MONTHS(TO_DATE('21-07-2023','DD-MM-YY'), -6)
                    AND tran_status = 'S' 
                    AND EXISTS (
                        SELECT 1 
                        FROM CINS_TMP_CUSTOMER_21072023 t2 
