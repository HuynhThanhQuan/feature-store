...
WHERE PROCESS_DT < TO_DATE('29-07-2023', 'DD-MM-YY')
  AND PROCESS_DT >= ADD_MONTHS(TO_DATE('29-07-2023', 'DD-MM-YY'), -6)
GROUP BY CUSTOMER_CDE;
...
WHERE CATEGORY_CDE LIKE '10__'
  AND CUSTOMER_CDE IN
    (SELECT CUSTOMER_CDE
     FROM CINS_TMP_CUSTOMER_29072023)
  AND ADD_MONTHS(TO_DATE('29-07-2023', 'DD-MM-YY'), -1) <= PROCESS_DT
  AND PROCESS_DT < TO_DATE('29-07-2023', 'DD-MM-YY')
GROUP BY CUSTOMER_CDE;
...
Derived From: CINS_TMP_CREDIT_CARD_TRANSACTION_29072023, CINS_TMP_DATA_RPT_CARD_29072023
*/
INSERT INTO CINS_FEATURE_STORE_V2
...
LEFT JOIN
  (SELECT CUSTOMER_CDE,
   ...
   FROM CINS_TMP_DATA_RPT_CARD_29072023
...
FROM
  (SELECT CUSTOMER_CDE,
   ...
   FROM CINS_TMP_CREDIT_CARD_TRANSACTION_29072023
   WHERE CUSTOMER_CDE IS NOT NULL
     AND ADD_MONTHS(TO_DATE('29-07-2023', 'DD-MM-YY'), -6) <= PROCESS_DT
     AND PROCESS_DT < TO_DATE('29-07-2023', 'DD-MM-YY')
   GROUP BY CUSTOMER_CDE) A
LEFT JOIN
  (SELECT CUSTOMER_CDE,
...
WITH A AS
  (SELECT CUSTOMER_CDE,
          ...
          SUM(CASE
              ...
              END) AS AMOUNT_SALE
   FROM CINS_TMP_CREDIT_CARD_TRANSACTION_29072023
   WHERE ADD_MONTHS(TO_DATE('29-07-2023', 'DD-MM-YY'), -6) <= PROCESS_DT
     AND PROCESS_DT < TO_DATE('29-07-2023', 'DD-MM-YY')
   GROUP BY CUSTOMER_CDE)
SELECT CUSTOMER_CDE,
...
Derived From: DW_CARD_TRANSACTION_FCT, CINS_TMP_CUSTOMER_29072023, DATA_RPT_CARD_493
*/
INSERT INTO CINS_FEATURE_STORE_V2 
WITH A AS
  (SELECT CUSTOMER_CDE,
...
WITH A AS
  (SELECT CUSTOMER_CDE,
   ...
   FROM CINS_TMP_CREDIT_CARD_TRANSACTION_29072023
   WHERE ADD_MONTHS(TO_DATE('29-07-2023', 'DD-MM-YY'), -3) <= PROCESS_DT
     AND PROCESS_DT < TO_DATE('29-07-2023', 'DD-MM-YY')
   GROUP BY CUSTOMER_CDE,
            PROCESS_DT)
...
FROM
  (SELECT CUSTOMER_CDE,
          SUM(CASE
                  WHEN (ADD_MONTHS(TO_DATE('29-07-2023', 'DD-MM-YY'), -1) <= PROCESS_DT
                        AND PROCESS_DT < TO_DATE('29-07-2023', 'DD-MM-YY')) THEN TICKET_SIZE
                  ELSE 0
              END) AS TICKET_SIZE_N0,
          SUM(CASE
                  WHEN (ADD_MONTHS(TO_DATE('29-07-2023', 'DD-MM-YY'), -2) <= PROCESS_DT
                        AND PROCESS_DT < ADD_MONTHS(TO_DATE('29-07-2023', 'DD-MM-YY'), -1)) THEN TICKET_SIZE
                  ELSE 0
              END) AS TICKET_SIZE_N1,
          SUM(CASE
                  WHEN (ADD_MONTHS(TO_DATE('29-07-2023', 'DD-MM-YY'), -3) <= PROCESS_DT
                        AND PROCESS_DT < ADD_MONTHS(TO_DATE('29-07-2023', 'DD-MM-YY'), -2)) THEN TICKET_SIZE
                  ELSE 0
