...
Derived From: CINS_TMP_POS_MERCHANT_6M_03082023, CINS_TMP_POS_TERMINAL_6M_03082023
*/
...
FROM
  (SELECT CUSTOMER_CDE,
   ...
   FROM (
           (SELECT *
            FROM CINS_TMP_POS_MERCHANT_6M_03082023
            WHERE to_date(RPT_DT, 'DD-MM-YY') = to_date('03-08-2023', 'DD-MM-YY') )
         UNION  ALL
           (SELECT CUSTOMER_CDE,
                   MERCHANT_CDE,
                   TERMINAL_ID,
                   ...
                   CT_TXN_TERMINAL,
                   ADD_TSTP
            FROM CINS_TMP_POS_TERMINAL_6M_03082023
            WHERE to_date(RPT_DT, 'DD-MM-YY') = to_date('03-08-2023', 'DD-MM-YY') )))
WHERE RN1 = 1;

/*
Feature Name: FAV_POS_6M_SM
Derived From: CINS_TMP_POS_TERMINAL_AMT_6M_03082023, CINS_TMP_POS_MERCHANT_AMT_6M_03082023
*/
INSERT INTO CINS_FEATURE_STORE_V2
SELECT CUSTOMER_CDE,
...
FROM
  (SELECT CUSTOMER_CDE,
   ...
   FROM (
           (SELECT *
            FROM CINS_TMP_POS_TERMINAL_AMT_6M_03082023
            WHERE to_date(RPT_DT, 'DD-MM-YY') = to_date('03-08-2023', 'DD-MM-YY'))
         UNION  ALL
           (SELECT *
            FROM CINS_TMP_POS_MERCHANT_AMT_6M_03082023
            WHERE to_date(RPT_DT, 'DD-MM-YY') = to_date('03-08-2023', 'DD-MM-YY'))))
WHERE RN1 = 1;
/*
Feature Name: LOR
...
SELECT CUSTOMER_CDE,
       'LOR' FTR_NM,
             TO_DATE('03-08-2023', 'DD-MM-YY') - TO_DATE(CUS_OPEN_DT) FTR_VAL,
             TO_DATE('03-08-2023', 'DD-MM-YY') AS RPT_DT,
             CURRENT_TIMESTAMP ADD_TSTP
...
WHERE SUB_SECTOR_CDE IN ('1700',
  ...
  AND ACTIVE = '1'
  AND COMPANY_KEY = '1'
  AND TO_DATE(CUS_OPEN_DT) <= TO_DATE('03-08-2023', 'DD-MM-YY') ;
  ...
  (SELECT CUSTOMER_CDE,
   ...
   FROM
     (SELECT CUSTOMER_CDE,
      ...
      WHERE RANK_SCORE = 1
        AND TO_DATE(DATE_1) < TO_DATE('03-08-2023', 'DD-MM-YY') )
   WHERE RN = 1 ) A
RIGHT JOIN
  (SELECT CUSTOMER_CDE
   FROM DW_ANALYTICS.DW_CUSTOMER_DIM
...
Derived From: DATA_RPT_CARD_493, CINS_TMP_CUSTOMER_03082023, CINS_TMP_CARD_DIM_03082023
*/
INSERT INTO CINS_FEATURE_STORE_V2
SELECT CUSTOMER_CDE,
...
WHERE CUSTOMER_CDE IN
  ...
  AND CARD_CDE IN
    (SELECT CARD_CDE
     FROM CINS_TMP_CARD_DIM_03082023)
...
SELECT CUSTOMER_CDE,
       ...
       AVG(TO_DATE('03-08-2023', 'DD-MM-YY') - TO_DATE(LAST_SIGNED_ON)) AS FTR_VAL,
       TO_DATE('03-08-2023', 'DD-MM-YY') AS RPT_DT,
       CURRENT_TIMESTAMP AS ADD_TSTP
FROM A
WHERE TO_DATE(LAST_SIGNED_ON) < TO_DATE('03-08-2023', 'DD-MM-YY')
  AND TO_DATE(LAST_SIGNED_ON) >= ADD_MONTHS(TO_DATE('03-08-2023', 'DD-MM-YY'), -36)
  AND RN = 1
GROUP BY CUSTOMER_CDE;

/*
Feature Name: CARD_CREDIT_MAX_BRAND_LIMIT
Derived From: DW_CARD_MASTER_DIM, CINS_TMP_DATA_RPT_CARD_03082023
*/
INSERT INTO CINS_FEATURE_STORE_V2 
WITH T AS
...
WITH A AS
  (SELECT DISTINCT CUSTOMER_CDE,
                   ...
                   END AS CREDIT_CLASS
   FROM DW_ANALYTICS.DW_CARD_MASTER_DIM
   WHERE ACTIVATION_DT <= TO_DATE('03-08-2023', 'DD-MM-YY')
     AND ACTIVATION_DT >= ADD_MONTHS(TO_DATE('03-08-2023', 'DD-MM-YY'), -36)
     AND SUBSTR(CARD_CDE, 1, 1) = '3'
     AND PLASTIC_CDE = ' '
     AND STATUS_CDE = ' '
     AND CUSTOMER_CDE <> '-1'
...
FROM
  (SELECT DISTINCT CUSTOMER_CDE
   ...
   WHERE SUBSTR(CARD_CDE, 1, 1) = '3'
     ...
     AND ACTIVATION_DT <= TO_DATE('03-08-2023', 'DD-MM-YY')
     AND CUSTOMER_CDE IN
       (SELECT CUSTOMER_CDE
        FROM CINS_TMP_CUSTOMER_03082023) );
...
FROM
  (SELECT CUSTOMER_CDE,
   ...
   FROM A
   WHERE OPEN_DT < TO_DATE('03-08-2023', 'DD-MM-YY') )
WHERE RN = 1;
...
Derived From: CINS_TMP_EB_MB_CROSSELL_03082023
*/
INSERT INTO CINS_FEATURE_STORE_V2
SELECT CUSTOMER_CDE,
...
FROM
  (SELECT CUSTOMER_CDE,
   ...
   FROM CINS_TMP_EB_MB_CROSSELL_03082023
   WHERE INPUT_DT < TO_DATE('03-08-2023', 'DD-MM-YY') )
WHERE RN = 1 ;
...
FROM
  (SELECT CUSTOMER_CDE,
   ...
   WHERE SUB_SECTOR_CDE IN ('1700',
     ...
     AND ACTIVE = '1'
     AND COMPANY_KEY = '1'
     AND UPDATE_DT <= TO_DATE('03-08-2023', 'DD-MM-YY') )A
JOIN
  (SELECT *
