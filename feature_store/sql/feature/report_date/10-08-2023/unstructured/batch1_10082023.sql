...
SELECT CUSTOMER_CDE,
       'EB_MBIB_DAY_SINCE_LTST_TXN' FTR_NM,
                                    TO_DATE('10-08-2023', 'DD-MM-YY') - NVL(MAX(TXN_DT), ADD_MONTHS(TO_DATE('10-08-2023', 'DD-MM-YY'), -36)) FTR_VAL,
                                    TO_DATE('10-08-2023', 'DD-MM-YY') AS RPT_DT,
                                    CURRENT_TIMESTAMP ADD_TSTP
FROM DW_ANALYTICS.DW_EB_TRANSACTION_FCT
WHERE TXN_DT >= ADD_MONTHS(TO_DATE('10-08-2023', 'DD-MM-YY'), -36)
  AND TXN_DT < TO_DATE('10-08-2023', 'DD-MM-YY')
  AND TXN_STATUS = 'SUC'
  AND CUSTOMER_CDE IN
    (SELECT CUSTOMER_CDE
  ...
  AND CUSTOMER_CDE IN
    (SELECT CUSTOMER_CDE
     ...
     WHERE TXN_STATUS = 'SUC'
       AND TXN_DT < ADD_MONTHS(TO_DATE('10-08-2023', 'DD-MM-YY'), -12)
       AND TXN_DT >= ADD_MONTHS(TO_DATE('10-08-2023', 'DD-MM-YY'), -36)
...
Derived From: DW_EWALL_TRANSACTION_FCT, CINS_TMP_CUSTOMER_10082023
*/
INSERT INTO CINS_FEATURE_STORE_V2
SELECT CUSTOMER_CDE,
       'EB_SACOMPAY_DAY_SINCE_LTST_TXN' FTR_NM,
                                        TO_DATE('10-08-2023', 'DD-MM-YY') - NVL(MAX(PROCESS_DT), ADD_MONTHS(TO_DATE('10-08-2023', 'DD-MM-YY'), -36)) FTR_VAL,
                                        TO_DATE('10-08-2023', 'DD-MM-YY') AS RPT_DT,
                                        CURRENT_TIMESTAMP ADD_TSTP
...
Derived From: DW_EWALL_TRANSACTION_FCT, CINS_TMP_CUSTOMER_10082023, DW_EWALL_USER_DIM
*/
INSERT INTO CINS_FEATURE_STORE_V2
SELECT CUSTOMER_CDE,
...
FROM
  (SELECT CUSTOMER_CDE,
          NVL(MONTHS_BETWEEN(LEAD(TXN_DT) OVER (PARTITION BY CUSTOMER_CDE
                                                ORDER BY TXN_DT), TXN_DT), MONTHS_BETWEEN(TO_DATE('10-08-2023', 'DD-MM-YY'), TXN_DT)) TXN_GAP
   FROM DW_ANALYTICS.DW_EWALL_TRANSACTION_FCT
...
WHERE PROCESS_DT >= ADD_MONTHS(TO_DATE('10-08-2023', 'DD-MM-YY'), -36)
  AND CARD_CDE LIKE '3%'
  AND TRAN_STATUS = 'S'
     ...
     WHERE DIM.CUSTOMER_CDE = FCT.CUSTOMER_CDE
       ...
       AND (MCC_CDE NOT IN (0,
            ...
            OR MCC_CDE IN (6010,
                           ...
                           6051))
       AND PROCESS_DT < TO_DATE('10-08-2023', 'DD-MM-YY')
       AND PROCESS_DT >= ADD_MONTHS(TO_DATE('10-08-2023', 'DD-MM-YY'), -12))
GROUP BY CUSTOMER_CDE;
...
FROM
  (SELECT CUSTOMER_CDE,
          NVL(MONTHS_BETWEEN(LEAD(PROCESS_DT) OVER (PARTITION BY CUSTOMER_CDE
                                                    ORDER BY PROCESS_DT), PROCESS_DT), MONTHS_BETWEEN(TO_DATE('10-08-2023', 'DD-MM-YY'), PROCESS_DT)) TXN_GAP
  ...
  AND CUSTOMER_CDE IN
    (SELECT CUSTOMER_CDE
     ...
     WHERE CARD_CDE LIKE '3%'
       AND STATUS_CDE = ' '
       AND ACTIVATION_DT < ADD_MONTHS(TO_DATE('10-08-2023', 'DD-MM-YY'), -3)
...
Derived From: DW_ACCOUNT_MASTER_DIM, DWA_STMT_EBANK, TRANSACTION_CODE, CINS_TMP_CUSTOMER_10082023
*/
INSERT INTO CINS_FEATURE_STORE_V2
SELECT CUSTOMER_CDE,
...
Derived From: DWA_STMT_EBANK, TRANSACTION_CODE, CINS_TMP_CUSTOMER_10082023
*/ 
INSERT INTO CINS_FEATURE_STORE_V2
...
WHERE ACTIVE = 1
  ...
  AND SUB_SECTOR_CDE IN ('1700',
                         '1602')
  AND CUS_OPEN_DT < TO_DATE('10-08-2023', 'DD-MM-YY')
  AND CUSTOMER_CDE NOT IN
    (SELECT CUSTOMER_ID
     FROM DW_ANALYTICS.DWA_STMT_EBANK TXN
     JOIN
...
SELECT CUSTOMER_CDE,
       'AGE' FTR_NM,
             FLOOR(MONTHS_BETWEEN(TO_DATE('10-08-2023', 'DD-MM-YY'), BIRTHDAY)/12) FTR_VAL,
             TO_DATE('10-08-2023', 'DD-MM-YY') AS RPT_DT,
             CURRENT_TIMESTAMP ADD_TSTP
FROM DW_ANALYTICS.DW_CUSTOMER_DIM
WHERE ACTIVE = 1
...
SELECT CUSTOMER_CDE,
       'LIFE_STG' FTR_NM,
                  CASE
                      WHEN FLOOR(MONTHS_BETWEEN(TO_DATE('10-08-2023', 'DD-MM-YY'), BIRTHDAY)/12) BETWEEN 18 AND 26 THEN 'Bắt đầu sự nghiệp'
                      WHEN FLOOR(MONTHS_BETWEEN(TO_DATE('10-08-2023', 'DD-MM-YY'), BIRTHDAY)/12) BETWEEN 27 AND 35 THEN 'Lập gia đình'
                      WHEN FLOOR(MONTHS_BETWEEN(TO_DATE('10-08-2023', 'DD-MM-YY'), BIRTHDAY)/12) BETWEEN 36 AND 45 THEN 'Thiết lập tài sản'
                      WHEN FLOOR(MONTHS_BETWEEN(TO_DATE('10-08-2023', 'DD-MM-YY'), BIRTHDAY)/12) BETWEEN 46 AND 54 THEN 'Bảo vệ tài sản'
                      WHEN FLOOR(MONTHS_BETWEEN(TO_DATE('10-08-2023', 'DD-MM-YY'), BIRTHDAY)/12) BETWEEN 55 AND 64 THEN 'Cuối sự nghiệp'
                      WHEN FLOOR(MONTHS_BETWEEN(TO_DATE('10-08-2023', 'DD-MM-YY'), BIRTHDAY)/12) >= 65 THEN 'Nghỉ hưu'
                      ELSE NULL
                  END FTR_VAL,
                  TO_DATE('10-08-2023', 'DD-MM-YY') AS RPT_DT,
                  CURRENT_TIMESTAMP ADD_TSTP
...
Derived From: CINS_TMP_CUSTOMER_STATUS_10082023
*/
INSERT INTO CINS_FEATURE_STORE_V2
SELECT CUSTOMER_CDE,
       'INACTIVE' FTR_NM,
       ...
       TO_DATE('10-08-2023', 'DD-MM-YY') AS RPT_DT,
       CURRENT_TIMESTAMP ADD_TSTP
FROM CINS_TMP_CUSTOMER_STATUS_10082023
WHERE CUST_STT >= 1
  AND RPT_DT = TO_DATE('10-08-2023', 'DD-MM-YY');
...
Derived From: DW_EB_USER, CINS_TMP_CUSTOMER_10082023
*/
INSERT INTO CINS_FEATURE_STORE_V2
...
SELECT CUSTOMER_CDE,
       'EB_MBIB_DAY_SINCE_ACTIVE' FTR_NM,
       TO_DATE('10-08-2023', 'DD-MM-YY') - ACTIVATE_DATE FTR_VAL,
       TO_DATE('10-08-2023', 'DD-MM-YY') AS RPT_DT,
       CURRENT_TIMESTAMP ADD_TSTP
FROM
  (SELECT CUSTOMER_CDE,
   ...
   WHERE EXISTS
       (SELECT 1
        FROM CINS_TMP_CUSTOMER_10082023 t2
        WHERE t1.CUSTOMER_CDE=t2.CUSTOMER_CDE)
     AND TO_DATE(ACTIVATE_DT) <= TO_DATE('10-08-2023', 'DD-MM-YY')
     AND ACTIVATE_DT != TO_DATE('01/01/2400', 'DD/MM/YYYY')
   GROUP BY customer_cde) A;