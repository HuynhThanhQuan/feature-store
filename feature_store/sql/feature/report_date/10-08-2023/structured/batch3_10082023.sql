      ...
      FROM
        (SELECT CUSTOMER_CDE,
         ...
         WHERE PROCESS_DT < TO_DATE('10-08-2023', 'DD-MM-YY')
           AND PROCESS_DT >= ADD_MONTHS(TO_DATE('10-08-2023', 'DD-MM-YY'), -1)
           AND tran_status = 'S'
           AND CUSTOMER_CDE IN
             (SELECT CUSTOMER_CDE
...
Derived From: DW_ACCOUNT_MASTER_DIM, CINS_TMP_CUSTOMER_10082023
*/
INSERT INTO CINS_FEATURE_STORE_V2
SELECT CUSTOMER_CDE,
...
FROM
  (SELECT CUSTOMER_CDE,
   ...
   WHERE CUSTOMER_CDE IN
     ...
     AND active = 1
     AND open_dt >= ADD_MONTHS(TO_DATE('10-08-2023', 'DD-MM-YY'), -3)
     AND company_key = 1
     AND active = 1
...
WHERE anniv_dt < TO_DATE('10-08-2023', 'DD-MM-YY')
  AND anniv_dt >= ADD_MONTHS(TO_DATE('10-08-2023', 'DD-MM-YY'), -3)
  AND CUSTOMER_CDE IN
    (SELECT CUSTOMER_CDE
     FROM CINS_TMP_CUSTOMER_10082023)
GROUP BY CUSTOMER_CDE;
...
Derived From: DW_EB_TRANSACTION_FCT, CINS_TMP_CUSTOMER_10082023
*/
INSERT INTO CINS_FEATURE_STORE_V2
SELECT CUSTOMER_CDE,
       'EB_CT_TXN_3M' FTR_NM,
...
WHERE TXN_DT < TO_DATE('10-08-2023', 'DD-MM-YY')
  AND TXN_DT >= ADD_MONTHS(TO_DATE('10-08-2023', 'DD-MM-YY'), -3)
GROUP BY CUSTOMER_CDE;
