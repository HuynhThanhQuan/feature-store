create or replace PROCEDURE ebanking_proc (RPT_DT in DATE  )
as 
...
(
SELECT a.CUSTOMER_CDE,
      NVL(MONTHS_BETWEEN(LEAD(TXN_DT) OVER (PARTITION BY a.CUSTOMER_CDE ORDER BY TXN_DT), TXN_DT), MONTHS_BETWEEN(TO_DATE(RPT_DT,'DD-MM-YY'), TXN_DT)) TXN_GAP
...
SELECT t1.CUSTOMER_CDE, 
    ...
    TO_DATE(RPT_DT,'DD-MM-YY') - NVL( MAX(TXN_DT), ADD_MONTHS(TO_DATE(RPT_DT,'DD-MM-YY'), -36)) FTR_VAL,-- chua hieu logic NVL
	TO_DATE(RPT_DT,'DD-MM-YY') AS RPT_DT,
    CURRENT_TIMESTAMP ADD_TSTP
