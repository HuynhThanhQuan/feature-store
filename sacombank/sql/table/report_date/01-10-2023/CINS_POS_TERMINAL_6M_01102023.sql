...
CREATE TABLE CINS_POS_TERMINAL_6M_01102023 (
...
/*
This script inserts data into the CINS_POS_TERMINAL_6M_01102023 table by selecting customer, merchant, and terminal information from the DW_CARD_TRANSACTION_FCT and DW_CARD_TERMINAL_DIM tables. 
...
The resulting data is then inserted into the CINS_POS_TERMINAL_6M_01102023 table along with the report date and current timestamp.
*/
INSERT INTO CINS_POS_TERMINAL_6M_01102023
select E.customer_cde,
    F.MERCHANT_CDE, F.TERMINAL_ID
    ,TO_CHAR(TO_DATE('01-10-2023','DD-MM-YY'), 'DD-MM-YYYY') AS RPT_DT, AMT_BILL ,CURRENT_TIMESTAMP ADD_TSTP  
FROM
(
...
(
 select customer_cde, trim(' ' from(merchant_cde)) merchant_cde, cardhdr_no, TERMINAL_ID,
        ...
        where process_dt < TO_DATE('01-10-2023','DD-MM-YY') AND process_dt >= ADD_MONTHS(TO_DATE('01-10-2023','DD-MM-YY'), -6)
        and tran_status = 'S' 
